...
namespace Pathfinding {
	[AddComponentMenu("Pathfinding/Modifiers/LastestFunnel")]
	[System.Serializable]
	...
	public class LastestFunnelModifier : MonoModifier {
	#if UNITY_EDITOR
		[UnityEditor.MenuItem("CONTEXT/Seeker/Add Lastest Funnel Modifier")]
		public static void AddComp (UnityEditor.MenuCommand command) {
			(command.context as Component).gameObject.AddComponent(typeof(LastestFunnelModifier));
		}
	#endif
		...
		public override void Apply (Path p) {
			...
			if (RunFunnel (left, right, funnelPath)) {
				...
				Debug.Log("LastestFunnelModifier RunFunnel Failed !");
				ListPool<Vector3>.Release (funnelPath);
			}
